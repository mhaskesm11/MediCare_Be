...
public interface ProductStockRepository extends JpaRepository<ProductStock, Long> {
	...
	@Modifying
	@Query
	(value = "update medicare_database.txn_product_stock set quantity=(quantity-?1) where product_name =?2",
			nativeQuery = true)
	void minusItemsFromStocks(Long quantity,String name);
	
	@Query
	(value = "select * from txn_product_stock where product_name=?1",
			nativeQuery = true)
	Optional<ProductStock> getExistingStock(String productName );

	@Query
	(value = "select * from txn_product_stock order by product_name",
			nativeQuery = true)
	List<ProductStock> getAllProductStocks();

	@Query
	(value = "select price from txn_product_stock where product_name=?1",
			nativeQuery = true)
	...
	@Query
	(value = "select * from txn_product_stock where is_active='Y' ",
			nativeQuery = true)
